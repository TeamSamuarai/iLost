...
class CoordinateConverter: NSObject {
    ...
    */
    private var tangentLatitudeLongitudeCoordinate: CLLocationCoordinate2D
    ...
    */
    func MKMapPointFromPDFPoint(pdfPoint: CGPoint) -> MKMapPoint {
        /*
        ...
        */
        let displacementFromTangentPoint = CGVectorMake(pdfPoint.x - tangentPDFPoint.x, pdfPoint.y - tangentPDFPoint.y)
    ...
    */
    func transformerFromPDFToMk() -> CGAffineTransform {
        ...
        */
        let resultOfTangentMercatorCoordinate = CGAffineTransformMakeTranslation(CGFloat(tangentMercatorCoordinate.x), CGFloat(tangentMercatorCoordinate.y))

        /*
            Revise the AffineTransform to first scale by 
            (1.0 / metersPerMapPoint), and then perform the above translation.
        */
        let resultOfEastSouthDistanceMeters = CGAffineTransformScale(resultOfTangentMercatorCoordinate, CGFloat(1.0 / metersPerMapPoint), CGFloat(1.0 / metersPerMapPoint))

        /*
            Revise the AffineTransform to first scale by 
            (1.0 / dotProduct(...)) before performing the transform so far.
        */
        let resultOfDotProduct = CGAffineTransformScale(resultOfEastSouthDistanceMeters,
            1.0 / oneMeterEastwardVector.dotProductWithVector(oneMeterEastwardVector),
            1.0 / oneMeterSouthwardVector.dotProductWithVector(oneMeterSouthwardVector))
        ...
        */
        let resultOfDisplacementFromTangentPoint = CGAffineTransformConcat(
            CGAffineTransformMake(
                oneMeterEastwardVector.dx, oneMeterEastwardVector.dy,
                oneMeterSouthwardVector.dx, oneMeterSouthwardVector.dy,
                0.0, 0.0
            ),
            resultOfDotProduct
        )
        ...
        */
        return CGAffineTransformTranslate(resultOfDisplacementFromTangentPoint, -tangentPDFPoint.x, -tangentPDFPoint.y)
    }
    ...
    */
    func polygonFromPDFRectCorners(pdfRect: CGRect) -> MKPolygon {
    ...
    /**
        - returns: the smallest MKMapRect that can show all rotations of the 
                given PDF rectangle.
    */
    func boundingMapRectIncludingRotations(rect: CGRect) -> MKMapRect {
